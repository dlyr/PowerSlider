...
//default value
	...
	connect(spinBox_, SIGNAL(valueChanged(double)), SLOT(on_spinBox_valueChanged(double)));
...
void  PowerSlider::on_slider_valueChanged(int val){
	...
	double dval = spinBox_->minimum()+
		double(val-slider_->minimum())/double(slider_->maximum()-slider_->minimum())*
...
void PowerSlider::on_spinBox_valueChanged(double val){
	...
	int ival = int(((val-spinBox_->minimum())/(spinBox_->maximum()-spinBox_->minimum())*(slider_->maximum()-slider_->minimum())));
	
	// update if needed    
	if(ival != slider_->value() )	{
...
void PowerSlider::setRange(double min, double max){
	int decimals = std::max(int(round(std::abs(log(min-floor(min))/log(10)))),
													int(round(std::abs(log(max-floor(max))/log(10)))))+1;
...
void PowerSlider::setSingleStep(double step){
	...
	slider_->setSingleStep(int(step/(spinBox_->maximum()-spinBox_->minimum())*double(slider_->maximum()-slider_->minimum())));	
}
...
double PowerSlider::minimum(){return spinBox_->minimum();}
double PowerSlider::maximum(){return spinBox_->maximum();}
double PowerSlider::singleStep(){return spinBox_->singleStep(); }

...
void PowerSlider::on_spinBox_valueChanged(double val) {
  ...
  int ival = int(((val - spinBox_->minimum()) /
                  (spinBox_->maximum() - spinBox_->minimum()) *
...
void PowerSlider::setRange(double min, double max) {
  int decimals =
      std::max(int(round(std::abs(log(min - floor(min)) / log(10)))),
...
void PowerSlider::setSingleStep(double step) {
  ...
  slider_->setSingleStep(
      int(step / (spinBox_->maximum() - spinBox_->minimum()) *
          double(slider_->maximum() - slider_->minimum())));
}
...
QDomElement PowerSlider::domElement(const QString &name,
...
double PowerSlider::value() { return spinBox_->value(); }
